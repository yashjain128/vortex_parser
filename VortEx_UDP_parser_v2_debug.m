...
% 0 for no write, 1 for write
writemode = 0;

% Set writeFile name
writeFile = 'writeTest01';
...
HK_TV = true;           % true to express HK in physical units, false for counts
...
if writemode
    try
        fid_w = fopen(['UDP_Exports\',writeFile,'.udp'],'a+');
    catch ME
        error('Check filename, could not create file.');
    end
end

% Flight E-box Identifiers
mNLP_ID = bin2dec('0001 0010');
EFP_ID = [bin2dec('1000'),bin2dec('0111')];
...
DATA_EFP = zeros(ceil(readlength/packetlength),3);
DATA_EFP_HK = zeros(floor(readlength/packetlength/11/2),11);
...
DATA_DIG_ACC_HIST = DATA_DIG_ACC;
...
DATA_EFP_Y = zeros(maxNumPoints,3);
DATA_EFP_HK_Y = zeros(maxNumPoints,11);
...
ylim12 = 1.1*[-2^11 2^11] + 2^11;

fig1 = figure(1);
% fig1.Units = 'normalized';
% fig1.Position(1:2) = [0.05,0.60];
subplot(2,3,3); 
mNLP_1_line = plot(1:maxNumPoints,DATA_mNLP_Y(:,1),lineStyle,'MarkerSize',3); xlabel('Sample Number'); ylabel('Counts'); title('mNLP'); hold on; xlim([1,maxNumPoints]); ylim(ylim24);
...
subplot(2,3,1); EFP_1_line = plot(1:maxNumPoints,DATA_EFP_Y(:,1),lineStyle,'MarkerSize',3); xlabel('Sample Number'); ylabel('Counts'); title('FPP'); hold on; xlim([1,maxNumPoints]); ylim(ylim20);
EFP_2_line = plot(1:maxNumPoints,DATA_EFP_Y(:,2),lineStyle,'MarkerSize',3);
subplot(2,3,4); EFP_3_line = plot(1:maxNumPoints,DATA_EFP_Y(:,3),lineStyle,'MarkerSize',3); xlabel('Sample Number'); ylabel('Counts'); title('EFP'); hold on; xlim([1,maxNumPoints]); ylim(ylim20);
...
subplot(2,3,6); D1_line = plot(1:maxNumPointsD,DATA_D_Y(:,1),lineStyle,'MarkerSize',3); xlabel('Sample Number'); ylabel('Counts'); title('Experiment D1/D2'); hold on; xlim([1,maxNumPointsD]); ylim(ylim12);
...
while (read_flag)
    ...
    elseif (readmode == 1)
        ...
        if writemode
            % write data
            fwrite(fid_w,rawdata(readlength-remainderPtr+2:readlength),'uint8');
        end
    end
    
    if writemode
        %close and reopen write file
        fclose(fid_w);
        fid_w =  fopen(['UDP_Exports\',writeFile,'.udp'],'a+');
    end
    %disp(packet_ind);
    if ~isempty(packet_ind)
        ...
        % sync is dropped.
        % For VortEx INT: TM engineer saw 100% frame lock and subframe
        % lock. This check actually 
        ...
        if(isempty(aa))
            ...
            disp(length(packet_ind));
            jj = packet_ind(packet_ind < remainderPtr);
            disp(length(jj));       
            numKAMframes = length(jj);
            TM_matrix(1:numKAMframes,:) = rawdata(jj' + kk);
            % disp(TM_matrix)
            % this is ONLY for debugging purposes
            TM_matrix_HIST = [TM_matrix_HIST; TM_matrix(1:numKAMframes,1:minorframelength)];
            ...
            jj = (1:exp_numpackets)';
            DATA_mNLP(jj,1) = 2^16*TM_matrix(aa,2*(2+5)-1) + 2^8*TM_matrix(aa,2*(2+5)) + TM_matrix(aa,2*(2+6)-1);
            DATA_mNLP(jj,2) = 2^16*TM_matrix(aa,2*(2+6)) + 2^8*TM_matrix(aa,2*(2+7)-1) + TM_matrix(aa,2*(2+7));
            DATA_mNLP(jj,3) = 2^16*TM_matrix(aa,2*(2+8)-1) + 2^8*TM_matrix(aa,2*(2+8)) + TM_matrix(aa,2*(2+9)-1);
            % disp(DATA_mNLP);
            % ghjk = input("wait");

            buffer_mNLP_HK(jj) = TM_matrix(aa,2*(2+9));
            DATA_EFP(jj,1) = 2^12*TM_matrix(aa,2*(2+15)-1) + 2^4*TM_matrix(aa,2*(2+15)) + bitand(bitshift(TM_matrix(aa,2*(2+18)-1),-4),15);
            DATA_EFP(jj,2) = 2^12*TM_matrix(aa,2*(2+16)-1) + 2^4*TM_matrix(aa,2*(2+16)) + bitand(TM_matrix(aa,2*(2+18)-1),15);
            DATA_EFP(jj,3) = 2^12*TM_matrix(aa,2*(2+17)-1) + 2^4*TM_matrix(aa,2*(2+17)) + bitand(bitshift(TM_matrix(aa,2*(2+18)),-4),15);
            buffer_EFP_HK(jj) = bitand(TM_matrix(aa,2*(2+18)),15); % 1 nibble only
            DATA_ACC(jj,1) = 2^10*TM_matrix(aa,2*(2+25)-1) + 2^2*TM_matrix(aa,2*(2+25)) + bitand(bitshift(TM_matrix(aa,2*(2+28)-1),-6),3);
            DATA_ACC(jj,2) = 2^10*TM_matrix(aa,2*(2+26)-1) + 2^2*TM_matrix(aa,2*(2+26)) + bitand(bitshift(TM_matrix(aa,2*(2+28)-1),-4),3);
            DATA_ACC(jj,3) = 2^10*TM_matrix(aa,2*(2+27)-1) + 2^2*TM_matrix(aa,2*(2+27)) + bitand(bitshift(TM_matrix(aa,2*(2+28)-1),-2),3);
            dig_acc_row1 = find(bitand(TM_matrix(aa,2*(2+28)-1),3)==1);
            dig_acc_row2 = find(bitand(TM_matrix(aa,2*(2+28)-1),3)==2);
            DATA_DIG_ACC(1:length(dig_acc_row1),1) = 2^12*TM_matrix(aa(dig_acc_row1),2*(2+28)) + 2^4*TM_matrix(aa(dig_acc_row1),2*(2+29)-1) + bitand(bitshift(TM_matrix(aa(dig_acc_row1),2*(2+30)),-4),15);
            DATA_DIG_ACC(1:length(dig_acc_row1),2) = 2^12*TM_matrix(aa(dig_acc_row1),2*(2+29)) + 2^4*TM_matrix(aa(dig_acc_row1),2*(2+30)-1) + bitand(TM_matrix(aa(dig_acc_row1),2*(2+30)),15);
            DATA_DIG_ACC(1:length(dig_acc_row2),3) = 2^12*TM_matrix(aa(dig_acc_row2),2*(2+28)) + 2^4*TM_matrix(aa(dig_acc_row2),2*(2+29)-1) + bitand(bitshift(TM_matrix(aa(dig_acc_row2),2*(2+29)),-4),15);
            DATA_DIG_ACC(1:length(dig_acc_row2),4) = 2^8*bitand(TM_matrix(aa(dig_acc_row2),2*(2+29)),15) + TM_matrix(aa(dig_acc_row2),2*(2+30)-1);
            buffer_ACC_HK(1:length(dig_acc_row2)) = TM_matrix(aa(dig_acc_row2),2*(2+30));
...
%             DATA_DIG_ACC_HIST = [DATA_DIG_ACC_HIST;DATA_DIG_ACC];
            
            sfid = TM_matrix(aa,2*(2+1));
            sfid_even = find(mod(sfid,2)==0);
            sfid_odd = find(mod(sfid,2)==1);
            % sfid runs 0->19: D1 is even, D2 is odd
            DATA_D(1:length(sfid_even),1) = (2^8*TM_matrix(aa(sfid_even),2*(2+38)-1) + TM_matrix(aa(sfid_even),2*(2+38)))/2^4;
            DATA_D(1:length(sfid_odd ),2) = (2^8*TM_matrix(aa(sfid_odd) ,2*(2+38)-1) + TM_matrix(aa(sfid_odd) ,2*(2+38)))/2^4;
            DATA_D(1:length(sfid_even),1) = (2^8*TM_matrix(sfid_even,2*(2+38)-1) + TM_matrix(sfid_even,2*(2+38)))/2^4;
            DATA_D(1:length(sfid_odd ),2) = (2^8*TM_matrix(sfid_odd ,2*(2+38)-1) + TM_matrix(sfid_odd ,2*(2+38)))/2^4;
            ...
            DATA_EFP = DATA_EFP - (DATA_EFP >= 2^19)*2^20;
            ...
            DATA_EFP_Y = circshift(DATA_EFP_Y,-exp_numpackets);
            ...
            DATA_EFP_Y((maxNumPoints-exp_numpackets+1):maxNumPoints,:) = DATA_EFP(1:exp_numpackets,:);
            ...
            % GPS
            GPS_columns(jj,1) = 2^8*TM_matrix(aa,2*(2+2)-1) + TM_matrix(aa,2*(2+2));
            GPS_columns(jj,2) = 2^8*TM_matrix(aa,2*(2+12)-1) + TM_matrix(aa,2*(2+12));
            GPS_columns(jj,3) = 2^8*TM_matrix(aa,2*(2+22)-1) + TM_matrix(aa,2*(2+22));
            GPS_columns(jj,4) = 2^8*TM_matrix(aa,2*(2+32)-1) + TM_matrix(aa,2*(2+32));
            
            GPS_onecolumn(1:exp_numpackets*4,1) = reshape(transpose(GPS_columns(jj,1:4)),exp_numpackets*4,1);
            ...
            if ~isempty(GPS_rV)
                ...
                for jj = 1:length(GPS_rV)
                    GPS_rV_crc(jj,1) = crc16_Dec(GPS_rV_packets(jj,1:rV_LENGTH-3));
                    GPS_rV_crc(jj,2) = 256*GPS_rV_packets(jj,rV_LENGTH-1) + GPS_rV_packets(jj,rV_LENGTH-2);
                end
            ...
            if (toc(plot_tic) >= plot_delay)
                ...
                if (plotfigure1)
                    ...
                    set(EFP_1_line,'YData',DATA_EFP_Y(:,1));
                    set(EFP_2_line,'YData',DATA_EFP_Y(:,2));
                    set(EFP_3_line,'YData',DATA_EFP_Y(:,3));
                ...
                if (HKparsing)
                    ...
                    if ~isempty(mNLP_HK_index)
                        ...
                        jj = 1:length(mNLP_HK_index);
                        DATA_mNLP_HK(jj,:) = buffer_mNLP_HK(mNLP_HK_index' + (0:HK_LENGTH-1));
                        % Add HK data to plotting buffer
                        DATA_mNLP_HK_Y = circshift(DATA_mNLP_HK_Y,-length(mNLP_HK_index));
                        DATA_mNLP_HK_Y((maxNumPoints-length(mNLP_HK_index)+1):maxNumPoints,:) = DATA_mNLP_HK(jj,:);
                    end
                    
                    % EFP
                    EFP_HK_index = strfind(buffer_EFP_HK',EFP_ID);
                    EFP_HK_index = EFP_HK_index(diff(EFP_HK_index) == 2*HK_LENGTH);
                    if ~isempty(EFP_HK_index)
                        % Extract all valid HK data
                        jj = 1:length(EFP_HK_index);
                        DATA_EFP_HK(jj,:) = 2^4*buffer_EFP_HK(EFP_HK_index' + (0:2:2*HK_LENGTH-2)) + ...
                            buffer_EFP_HK(EFP_HK_index' + (1:2:2*HK_LENGTH-1));
                        % Add HK data to plotting buffer
                        DATA_EFP_HK_Y = circshift(DATA_EFP_HK_Y,-length(EFP_HK_index));
                        DATA_EFP_HK_Y((maxNumPoints-length(EFP_HK_index)+1):maxNumPoints,:) = DATA_EFP_HK(jj,:);
                    end
                    ...
                    if ~isempty(ACC_HK_index)
                        % Extract all valid HK data
                        jj = 1:length(ACC_HK_index);
                        DATA_ACC_HK(jj,:) = buffer_ACC_HK(ACC_HK_index' + (0:HK_LENGTH-1));
                        % Add HK data to plotting buffer
                        DATA_ACC_HK_Y = circshift(DATA_ACC_HK_Y,-length(ACC_HK_index));
                        DATA_ACC_HK_Y((maxNumPoints-length(ACC_HK_index)+1):maxNumPoints,:) = DATA_ACC_HK(jj,:);
                    end
                    ...
                    if HK_TV
                        ...
                        DATA_EFP_HK_Y((maxNumPoints-length(EFP_HK_index)+1):maxNumPoints,2:5) = -76.9231* (DATA_EFP_HK(1:length(EFP_HK_index),2:5) *2.5/256 - 0.5*2.5/256) + 202.54;
                        DATA_EFP_HK_Y((maxNumPoints-length(EFP_HK_index)+1):maxNumPoints,6:11) = [16,6.15,7.329,3,2,2].*(DATA_EFP_HK(1:length(EFP_HK_index),6:11) *2.5/256 - 0.5*2.5/256) + [0,-16.88,0,0,0,0];
